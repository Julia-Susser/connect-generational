...
def login():
    ...
    conversation = get_chatroom(name)
    try:
        conversation.participants.create(identity="hey")
...
@app.route('/advocate-contact')
def advocateContact():
    name = request.values.get('name')
    return render_template('advocate-contact.html', name=name)
